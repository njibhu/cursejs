    ...
    run(login, password){
        ...
        this.on(Events.CONNECTED, function(){
            self._loadContacts(function(errors){
                if(errors === null){
                    self._initWSSession(function(error){
                        if(error === null){
                            //Start the websocket when we have the session authorization for websockets
                            self._openWS();
                        }
                        else {
                            winston.log('error', 'Client.run', 'Cannot start websocket', errors);
                        }
                    });
                }
                else {
