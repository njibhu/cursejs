    ...
    login(login, password){
        ...
        loginEndpoint.login(this._loginRequest, function(answer){
            if(answer.Status == 1){
                ...
                winston.log('error', 'Client.login:', 'Status:', answer.Status);
                winston.log('error', 'Client.login:', 'StatusMessage:', answer.StatusMessage);
            }
        });
    }
    ...
    _initWSSession(){
        ...
        sessionsEndpoint.sessions(sessionRequest, this._loginSession['Token'], function(answer){
            if(typeof(answer['error']) == 'undefined'){
                ...
                winston.log('error', 'Client._initWSSession:', 'Status:', answer['error']);
            }
        });
    }
