...
class Server {
    ...
    everyServerMembers(callback){
        ...
        var requestActiveMembers = true;
        function requestCallback(){
            groupsEndpoints.getGroupMembers(self.ID, requestActiveMembers, page, 50, self.client.token, function(errors, answer){
                if(errors === null){
                    userArray = answer.content;

                    //What if endpoints return empty array ?
                    if(userArray.length == 0){
                        //Reloop for non active members after done looping through all actives ones
                        if(requestActiveMembers){
                            requestActiveMembers = false;
                            page = 1;
                            requestCallback();
                        }else {
                            continueLoop = false;
